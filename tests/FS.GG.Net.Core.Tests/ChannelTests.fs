    ...
    testList
        ...
        [
            ...
            <| async {
                ...
                let! result =
                    exchange.WaitAsync(TimeSpan.FromSeconds 5.0) |> Async.AwaitTask |> Async.Catch
            ...
            <| async {
                ...
                for exchangeResult in exchangeResults do
                    ...
                    | Choice2Of2 ex when (unwrap ex :? MessageChannelClosed) -> ()
                    | Choice2Of2 ex -> failtestf "expected MessageChannelClosed, got %O" ex
                    | Choice1Of2 _ -> failtest "an admitted exchange must settle when the channel closes"
                ...
                match rejected with
                | Choice2Of2 ex when (unwrap ex :? MessageChannelClosed) -> ()
                | Choice2Of2 ex -> failtestf "expected MessageChannelClosed after admission stopped, got %O" ex
            ...
            <| async {
                ...
                let serving =
                    MessageChannel.serve (transport :> ITransport) codec codec None (fun request ->
            ...
            <| async {
                ...
                let! observed =
                    diagnostic.Task.WaitAsync(TimeSpan.FromSeconds 5.0) |> Async.AwaitTask
            ...
            <| async {
                ...
                let handler (req: Env) : Task<Env> =
                    Task.FromResult { req with Text = "ok:" + req.Text }

                let _serveLoop =
                    MessageChannel.serve serverEnd codec codec (Some serverEcho) handler

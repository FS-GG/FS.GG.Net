...
module DeliveryTests =
    ...
    let private create maxQueue : DeliveryState<string, string> =
        Delivery.create
            {
                MaxQueuedDeltas = maxQueue
                ReconnectLifetimeMilliseconds = 50UL
            }
            ticket
            0UL
    ...
    let private success =
        function
        | Ok value -> value
        | Error issue -> failtestf "%A" issue

    [<Tests>]
    let tests =
        testList
            "bounded delivery and reconnect"
            [
                testCase "delayed acknowledgement cannot discard a newer delta"
                <| fun _ ->
                    ...
                    Expect.equal
                        ...
                        [ DeliveryMessage.Delta(2UL, "d2") ]
                        "only the acknowledged prefix is removed"

                    Expect.equal
                        (Delivery.acknowledge 3UL acknowledged)
                        (Error(DeliveryIssue.AcknowledgementAhead(2UL, 3UL)))
                        "client acknowledgement cannot lead authority"

                testCase "queue saturation coalesces to the current full snapshot"
                <| fun _ ->
                    ...
                    Expect.equal
                        (Delivery.pending three)
                        [ DeliveryMessage.Snapshot(3UL, "s3") ]
                        "bounded history becomes one authoritative snapshot"

                    Expect.equal
                        effect
                        (DeliveryEffect.QueueCoalesced(DeliveryMessage.Snapshot(3UL, "s3")))
                        "backpressure is observable"

                testCase "reconnect validates identity token and expiry"
                <| fun _ ->
                    ...
                    Expect.equal
                        (disconnected |> Delivery.reconnect { ticket with Token = "wrong" } 120UL)
                        (Error DeliveryIssue.TokenMismatch)
                        "wrong token is refused"

                    Expect.equal
                        (disconnected |> Delivery.reconnect ticket 151UL)
                        (Error(DeliveryIssue.ReconnectExpired(150UL, 151UL)))
                        "expired reconnect is refused"

                testCase "dispose clears pending work and makes later operations inert"
                <| fun _ ->
                    ...
                    Expect.equal
                        (disposed |> Delivery.publish 2UL "d2" "s2")
                        (Error DeliveryIssue.Disposed)
                        "disposed delivery refuses publication"

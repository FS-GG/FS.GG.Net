...
module Delivery =
    ...
    let private messageRevision =
        function
        | DeliveryMessage.Delta(value, _)
        | DeliveryMessage.Snapshot(value, _) -> value

    let create config ticket initialRevision =
        let issues =
            [
                if config.MaxQueuedDeltas < 1 then
                    DeliveryIssue.InvalidMaximumQueue config.MaxQueuedDeltas
                if missing ticket.SessionId then
                    DeliveryIssue.MissingSessionId
                if missing ticket.ClientId then
                    DeliveryIssue.MissingClientId
                if missing ticket.Token then
                    DeliveryIssue.MissingToken
            ]
    ...
    let publish revision delta snapshot state =
        ...
        if status = DeliveryStatus.Disposed then
            Error DeliveryIssue.Disposed
        elif revision <= current then
            Error(DeliveryIssue.RevisionNotMonotonic(current, revision))
        else
    ...
    let acknowledge
        ...
        if status = DeliveryStatus.Disposed then
            ...
            Error(DeliveryIssue.AcknowledgementAhead(current, revision))
        else
            let retained = pending |> List.filter (messageRevision >> (<) revision)
            let nextAcknowledged = max acknowledged revision

            Ok(
                DeliveryState(config, ticket, status, current, nextAcknowledged, retained),
                DeliveryEffect.QueueAcknowledged nextAcknowledged
            )
    ...
    let disconnect
        ...
        if status = DeliveryStatus.Disposed then
            ...
            Ok(
                DeliveryState(config, ticket, DeliveryStatus.Disconnected expires, current, acknowledged, pending),
                DeliveryEffect.DisconnectedUntil expires
            )
    ...
    let reconnect
        ...
        (state: DeliveryState<'delta, 'snapshot>)
        : Result<DeliveryState<'delta, 'snapshot> * DeliveryEffect<'delta, 'snapshot>, DeliveryIssue> =
        let (DeliveryState(config, expected, status, current, acknowledged, pending)) =
            state

        if status = DeliveryStatus.Disposed then
            Error DeliveryIssue.Disposed
        elif ticket.SessionId <> expected.SessionId then
            Error(DeliveryIssue.WrongSession(expected.SessionId, ticket.SessionId))
        elif ticket.ClientId <> expected.ClientId then
            Error(DeliveryIssue.WrongClient(expected.ClientId, ticket.ClientId))
        elif ticket.Token <> expected.Token then
            Error DeliveryIssue.TokenMismatch
        else
            match status with
            | DeliveryStatus.Disconnected expires when nowMilliseconds > expires ->
                Error(DeliveryIssue.ReconnectExpired(expires, nowMilliseconds))
            | _ ->
                Ok(
                    DeliveryState(config, expected, DeliveryStatus.Connected, current, acknowledged, pending),
                    DeliveryEffect.Reconnected pending
                )

    let dispose
        (DeliveryState(config, ticket, _, current, acknowledged, _): DeliveryState<'delta, 'snapshot>)
        : DeliveryState<'delta, 'snapshot> * DeliveryEffect<'delta, 'snapshot> =

    ...
    testList
        ...
        [
            ...
            <| async {
                ...
                do!
                    transport.Send(ReadOnlyMemory big, CancellationToken.None).AsTask()
                    |> Async.AwaitTask
            ...
            <| async {
                ...
                let! closeStatus =
                    server.PeerCloseStatus.WaitAsync(TimeSpan.FromSeconds 5.0) |> Async.AwaitTask
            ...
            <| async {
                ...
                for message in messages do
                    do!
                        transport
                            .Send(ReadOnlyMemory(Text.Encoding.UTF8.GetBytes message), CancellationToken.None)
                            .AsTask()
                        |> Async.AwaitTask
            ...
            <| async {
                ...
                let! resp =
                    channel.Exchange(StringValue(Value = "ping"), CancellationToken.None)
                    |> Async.AwaitTask
            ...
            <| async {
                ...
                let tcs =
                    TaskCompletionSource<Msg>(TaskCreationOptions.RunContinuationsAsynchronously)
            ...
            <| async {
                ...
                let _, start =
                    Net.Sub.incoming channel "dispose-tracking" Msg.Got |> List.exactlyOne
            ...
            <| async {
                ...
                let e =
                    greeter.CountTo({ To = 3 }, CallContext.Default).GetAsyncEnumerator(CancellationToken.None)
            ...
            <| async {
                ...
                let! server =
                    WebSocketServer.start (Uri "ws://127.0.0.1:0/") WebSocketOptions.defaults (fun transport ->
                        MessageChannel.serve transport codec codec None handler)
                    |> Async.AwaitTask
                ...
                let! r1 =
                    channel.Exchange(StringValue(Value = "one"), CancellationToken.None)
                    |> Async.AwaitTask

                let! r2 =
                    channel.Exchange(StringValue(Value = "two"), CancellationToken.None)
                    |> Async.AwaitTask
